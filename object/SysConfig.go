...
func (i *systemConfigIris) FormatConfig() {
	...
	i.LogLevel = i.checkLogLevel(i.LogLevel)
}

//校验SysConfig中iris日志级别设置（Server|Client）
func (i *systemConfigIris) checkLogLevel(level string) string {
	switch level {
	case "debug", "info", "warn", "error":
		return level
	default:
...
func (sc *systemConfigService) FormatConfig() {
	...
	if sc.Name == "" {
		sc.Name = "goTextHandle"
	}
	if sc.DisplayName == "" {
		sc.DisplayName = "goTextHandle"
	}
	if sc.Description == "" {
